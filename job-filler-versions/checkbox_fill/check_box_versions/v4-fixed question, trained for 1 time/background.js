// background.js (MV3)
// Requires permissions: ["scripting","activeTab"], and a command "fill-fixed" bound to Alt+2.
    ...
    func: () => {
      (async function () {
        ...
        const QA = [
          ...
          ["Please select your gender", "Male"], // <-- fixed matcher prevents Male->Female
        ...
        // ----- Matchers (fixed) -----
        const YES = new Set(["yes","y","yes, i consent","yes i consent"]);
        const NO  = new Set(["no","n","opt-out","opt out","opt-out from receiving text messages","opt out from receiving text messages"]);

        function wordBoundaryMatch(a, b) {
          // test that b appears in a, bounded by non-letters (prevents "male" matching "female")
        ...
        function matchesAnswer(text, target) {
          ...
          if (YES.has(b)) return YES.has(a);
          if (NO.has(b))  return NO.has(a);

          // Exact first
          if (a === b) return true;

          // Boundary-aware contains either way (to support very long labels)
          if (wordBoundaryMatch(a, b)) return true;
          if (wordBoundaryMatch(b, a)) return true;

          return false;
        }

        // Wait for a question container to mount (SPA page changes)
        ...
        function findContainerStrict(question) {
          ...
          for (const el of candidates) {
            ...
            if (inNav(el)) continue; // never pick header/nav
        ...
        function setRadioOrCheckbox(container, targetText) {
          // Exact label match first, then boundary-aware
          const inputs = [
            ...queryAllDeep('input[type="radio"]', container),
            ...queryAllDeep('input[type="checkbox"]', container)
          ];
          const labelOf = (inp) => {
            if (inp.id) {
              const lab = container.querySelector(`label[for="${CSS.escape(inp.id)}"]`);
          ...
          // Rank inputs so exact == top
          const ranked = inputs.map(inp => {
            const txt = labelOf(inp);
            const A = norm(txt), B = norm(targetText);
            const exact = A === B ? 1 : 0;
            const boundary = !exact && (wordBoundaryMatch(A, B) || wordBoundaryMatch(B, A)) ? 1 : 0;
            return { inp, txt, exact, boundary };
          }).sort((x, y) => (y.exact - x.exact) || (y.boundary - x.boundary));

          for (const { inp, txt, exact, boundary } of ranked) {
            if (!exact && !boundary) continue;
            if (!inp.checked) { inp.scrollIntoView({ block: "center" }); realClick(inp); inp.dispatchEvent(new Event("change", { bubbles: true })); }
            return true;
          }
          return false;
        }
        ...
        function clickTargetFromOpenList(trigger, targetText) {
          ...
          const options = queryAllDeep('[data-automation-id="select-option"], [role="option"]', document)
            ...
            .filter(x => matchesAnswer(x.txt, targetText))
            .sort((a, b) => a.dist - b.dist);

          if (!options.length) return false;
          const optEl = options[0].o.closest('[role="option"], [data-automation-id="select-option"]') || options[0].o;
        ...
        for (const [q, ans] of QA) {
          const container = await waitForContainer(q, 5000);
          if (!container) { results.push(`${q.slice(0, 22)}… ✗`); continue; }

          container.scrollIntoView({ block: "center" });
          await sleep(80);

          const ok = await chooseValue(container, ans);
