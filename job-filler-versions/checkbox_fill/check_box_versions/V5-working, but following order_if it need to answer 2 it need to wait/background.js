  ...
  await chrome.scripting.executeScript({
    ...
    func: () => {
      (async function () {
        ...
        // --- FIX: make only gender matching boundary-aware (so "male" won't match "female")
        ...
        function matchesAnswer(text, target) {
          ...
          // Gender-only strict token match (prevents "female" from matching "male")
        ...
        function findContainerStrict(question) {
          ...
          for (const el of candidates) {
            ...
            // prefer compact containers near the text
        ...
        function clickTargetFromOpenList(trigger, targetText) {
          ...
          // rank by distance to the trigger (helps avoid clicking Language menu items)
        ...
        for (const [q, ans] of QA) {
          // Wait up to 5s for the question to appear (SPA page transitions)
