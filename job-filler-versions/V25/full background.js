// not test having problem, where it filled the page then it remove it.
...
function pageFillFn_TEXT_ONLY_DEFAULTS(data) {
  ...
  // Only keeps visible text fields. Filters out: hidden fields, off-screen fields, buttons, checkboxes, radio inputs
  const visible = (el) => {
  ...
  // Matcher: decides which field is which (name, email, etc.)
  const find = (keys) =>
    inputs.find((el) => {
  ...
  (async function () {
    ...
    function findContainerStrict(question) {
      ...
      for (const el of candidates) {
        ...
        const hasTextControl = !!el.querySelector(
          'input:not([type]), input[type="text"], input[type="email"], input[type="tel"], input[type="url"], input[type="search"], input[type="number"], textarea, [contenteditable=""], [contenteditable="true"]'
        );
...
// ================== USER CHOICE Q&A (robust; updated logic) ==================
  ...
  (async function () {
    ...
    function openDropdown(container) {
      const triggers = [
        ...
        ...queryAllDeep('[data-testid="select-trigger"]', container),   // Ashby-ish
    ...
    async function chooseValue(container, targetText) {
      try {
        // 1) Native <select>
        if (setNativeSelect(container, targetText)) {
          await sleep(100);
          return matchesAnswer(readSelectedText(container), targetText);
        }

        // 2) Greenhouse / Select2 style widgets
        if (setGreenhouseSelect(container, targetText)) {
          await sleep(140);
          return matchesAnswer(readSelectedText(container), targetText);
        }

        // 3) Radio / checkbox groups
        if (setRadioOrCheckbox(container, targetText)) {
          await sleep(100);
          return matchesAnswer(readSelectedText(container), targetText);
        }
        ...
        // 5) Workday / Ashby dropdowns: open + pick from floating listbox
        ...
        if (!acted) {
          await sleep(180);
          acted = clickTargetFromOpenList(trigger, targetText);
        }
        if (!acted) {
          acted = await typeThenEnter(targetText);
        }
        if (acted) {
          await sleep(160);
