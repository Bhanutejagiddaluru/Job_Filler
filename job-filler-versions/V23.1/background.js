...
function pageFillFn_TEXT_ONLY_DEFAULTS(data) {
  ...
  //  Version 1: V1: only if you’re targeting very simple, static forms and want the leanest matcher.
  // const find = (keys) =>
  //   inputs.find((el) => {
  //     const combined = [
...
chrome.commands.onCommand.addListener(async (command) => {
  ...
  // Capture highlighted question text
  const [{ result: selectedQuestion }] = await chrome.scripting.executeScript({
    target: { tabId: tab.id },
    func: () => window.getSelection().toString()
  });

  // Capture the SELECTED ANSWER from the page
  const [{ result: selectedAnswer }] = await chrome.scripting.executeScript({
    ...
    func: () => {
      ...
      // 1) Find node containing selected question text
      let questionNode = null;
      const allNodes = [...document.querySelectorAll("*:not(script):not(style)")];
      for (const el of allNodes) {
        if (el.innerText && el.innerText.includes(selectedText)) {
          questionNode = el;
          break;
        }
      }
      ...
      // 2) Find closest clean container
      let container =
        questionNode.closest("fieldset, .field, .form-group, div, section") ||
        questionNode.parentElement;
      ...
      // ------------------------
      // 3) FIRST PASS: Inside same container
      // ------------------------
      function findAnswerInside(el) {
        let ans = "";
        ...
        el.querySelectorAll("input[type='radio']:checked, input[type='checkbox']:checked")
          .forEach(inp => {
            let txt = "";
            if (inp.id) {
              const lbl = el.querySelector(`label[for='${inp.id}']`);
              if (lbl) txt = lbl.innerText.trim();
            }
            if (!txt && inp.closest("label"))
              txt = inp.closest("label").innerText.trim();
            if (txt) ans = txt;
          });
        ...
        el.querySelectorAll("select").forEach(sel => {
          const opt = sel.options[sel.selectedIndex];
          if (opt && opt.innerText.trim()) {
            ans = opt.innerText.trim();
          }
        });
      ...
      answer = findAnswerInside(container);

      // ------------------------
      // 4) SECOND PASS: sibling container (Workday often separates label & options)
      // ------------------------
      if (!answer) {
        const sib = container.nextElementSibling;
        if (sib) answer = findAnswerInside(sib);
      }

      // ------------------------
      // 5) THIRD PASS: fallback → search up the tree
      // ------------------------
      if (!answer) {
        let up = container.parentElement;
        for (let i = 0; i < 3 && up; i++) {
          const found = findAnswerInside(up);
          if (found) {
            answer = found;
            break;
          }
      ...
      // ------------------------
      // 6) FINAL FALLBACK → last selected option on page
      // ------------------------
      if (!answer) {
        ...
        if (checked) {
          if (checked.id) {
            ...
            if (lbl) answer = lbl.innerText.trim();
          } else if (checked.closest("label")) {
            answer = checked.closest("label").innerText.trim();
          }
        }
      }

      // If dropdown is selected globally
      if (!answer) {
        const globalSelect = document.querySelector("select");
        if (globalSelect) {
          const opt = globalSelect.options[globalSelect.selectedIndex];
          if (opt && opt.innerText.trim()) answer = opt.innerText.trim();
        }
      }
